...
En plus des opérateurs \macro{iff} et \macro{implies} proposés par \LaTeX{}, il a été ajouté l'opérateur \macro{becauseof} pour obtenir $\becauseof$
\footnote{
...
L'environnement \env{stepcalc} avec les options \verb+style = ar+ et \verb+style = ar*+ imprime une rédaction plus classique tout en utilisant des flèches pour indiquer les explications
...
\begin{demotab*}
    ...
    \demostep{} % --> Ne pas oublier {} ici !
                %     On commence par une cellule vide...
      & Voir l'énoncé.
      & $AB = 10 \, cm$
    \demostep
...
\begin{demotab*}[start = last]
    \demostep[demo-first-geo-fact]
      ...
      & Définition d'un triangle \newline équilatéral. 
      & $AB = BC = AC$
    \demostep[known-data] % --> Pas besoin d'utiliser {} ici
                          %     car on utilise l'argument optionnel.
      & Voir l'énoncé.
      & $AB = 10 \, cm$
    \demostep
        Voir les conséquences \newline
...
\newparaexample{Indiquer qu'une case n'a pas besoin d'être remplie} 

Dans les exemples précédents, nous n'avons rien à indiquer dans la case \emph{\og Je sais que... \fg} de la 2\ieme{} ligne.
On peut indiquer ceci explicitement et de façon symbolique avec le texte \txtdemoNOTHING{} mis en forme via  par la macro \macro{explnothing}
\footnote{
...
La macro \macro{demoxspace} est aussi proposée par l'environnement \env{demotab*}. Voici un exemple.
...
Un tableau devant utiliser plusieurs pages sera scindé comme dans l'image page \pageref{tnslog:demotab*-splitted} sans perte d'information
\footnote{
	Ici aussi tout le travail est fait par l'environnement \env{longtable} du package éponyme.
}.
...
% Changes shown - START
    ...
    \medskip
    \item[2021-03-27] Nouvelle version sous-mineure \verb+1.2.0-beta+.
...
% ------------------------ %
    ...
    \medskip
    \item[2021-03-22] Nouvelle version sous-mineure \verb+1.1.1-beta+.
...
% ------------------------ %
    ...
    \medskip
    \item[2021-03-04] Nouvelle version sous-mineure \verb+1.1.0-beta+.
...
\IDkey{cwidth} la largeur souhaitée pour la colonne centrale.
...
\IDmacro[n]{txtdemoNOTHING}   \hfill \prefix{NOTHING to indicate}
